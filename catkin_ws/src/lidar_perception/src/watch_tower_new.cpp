...
WatchTower::WatchTower(ros::NodeHandle &n){
  ...
  pos_pub = nh.advertise<geometry_msgs::PoseArray> ("/pos", 1);
...
void WatchTower::tracking(float x_a, float y_a, float x_b, float y_b){
  ...
  geometry_msgs::Pose p_a;
  geometry_msgs::Pose p_b;
  ...
  if (dis_a_1 == 10e5 && dis_a_2 == 10e5){
    geometry_msgs::PoseStamped p;
    p.pose.position.x = past_a_x;
    p.pose.position.y = past_a_y;
    a_path.poses.push_back(p);
    p_a = p.pose;
  }
  if (dis_b_1 == 10e5 && dis_b_2 == 10e5){
    geometry_msgs::PoseStamped p;
    p.pose.position.x = past_b_x;
    p.pose.position.y = past_b_y;
    b_path.poses.push_back(p);
    p_b = p.pose;
  }
  pos_arr.poses.push_back(p_a);
  pos_arr.poses.push_back(p_b);
  pos_pub.publish(pos_arr);
